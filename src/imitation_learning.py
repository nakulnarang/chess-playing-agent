...
class ChessDataset(Dataset):
    def __init__(self, states_file, actions_file):
        ...
        assert len(self.states) == len(self.actions), "States and actions must have the same number of samples."
...
def train_imitation_learning(
    ...
    for epoch in range(epochs):
        ...
        for states, start_squares, end_squares in tqdm(train_loader, desc=f"Epoch {epoch+1}/{epochs} Training"):
            states, start_squares, end_squares = states.to(device), start_squares.to(device), end_squares.to(device)
        ...
        print(f"Epoch {epoch+1} Train Loss: {epoch_train_loss:.4f}, Start Acc: {epoch_train_start_acc:.4f}, End Acc: {epoch_train_end_acc:.4f}")
        ...
        with torch.no_grad():
            for states, start_squares, end_squares in tqdm(val_loader, desc=f"Epoch {epoch+1}/{epochs} Validation"):
                states, start_squares, end_squares = states.to(device), start_squares.to(device), end_squares.to(device)
        ...
        print(f"Epoch {epoch+1} Val Loss: {epoch_val_loss:.4f}, Start Acc: {epoch_val_start_acc:.4f}, End Acc: {epoch_val_end_acc:.4f}")
...
def main():
    ...
    val_loader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=os.cpu_count() // 2 if os.cpu_count() else 0)
    ...
    # Train the model
    train_imitation_learning(model, train_loader, val_loader, optimizer, epochs=10, device=device)
